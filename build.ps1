...
cmake -B build . -DCMAKE_CXX_FLAGS="/EHsc" -DLAPACK_LIBRARIES="openblas" -DBLAS_LIBRARIES="openblas" -DBLA_VENDOR=OpenBLAS -DFAISS_ENABLE_GPU=ON -DFAISS_ENABLE_PYTHON=ON -DBUILD_SHARED_LIBS=ON -DFAISS_ENABLE_C_API=ON -DBUILD_TESTING=OFF
cd build

cp -f ../openblas/lib/libopenblas.lib faiss/openblas.lib
